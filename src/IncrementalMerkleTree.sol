...
contract IncrementalMerkleTree {
    ...
    mapping(uint256=>bytes32) public s_root;          // to store the root of the merkle tree
    uint32 public constant ROOT_HISTORY_SIZE = 30;
    uint32 public s_currentRootIndex;
    Poseidon2 public immutable i_hasher;
    uint32 public s_nextLeafIndex;
    bytes32[32] public s_cachedSubtrees; 


    error IncrementalMerkleTree_DepthShouldBeGreaterThanZero();
    error IncrementalMerkleTree_DepthShoulBeLessThan32();
    error IncrementalMerkleTree__IndexOutOfBounds(uint256 depth );
    ...
    constructor(uint32 _depth, Poseidon2 _hasher){
        ...
        if (_depth>=32){
            revert IncrementalMerkleTree_DepthShoulBeLessThan32();
        }
        i_depth=_depth;
        i_hasher= _hasher;

        s_root[0]= zeros(_depth);     //0 root
    }

    

    function _insert (bytes32 _leaf) internal returns(uint32) {
        //we need to add a new leaf to the merkle tree
        uint32 _nextLeafIndex = s_nextLeafIndex;
        // check whether the index of leaf is within the bound
        ...
        //figure out if the index is even or odd
            //-if even we need to put it on the left of hash and the zero tree on the right
                //store the result as the cached sub tree
            //-else if odd we need to put it on the right on the hash and cache tree of appropriate depth on the left
